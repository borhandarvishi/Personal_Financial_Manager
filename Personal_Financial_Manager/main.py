...
class CSV:
    ...
    def get_transactions(cls, start_date, end_date):
        ...
        if filtered_df.empty:
            ...
            print("\nSummary:")
            print(f"Total Income: {total_income:.2f} T")
            print(f"Total Expense: {total_expense:.2f} T")
            print(f"Net Savings: {(total_income - total_expense): .2f} T")
...
def add():
    CSV.initialize_csv()
    date = get_date("Enter the date of the transaction (dd-mm-yyyy) or enter for today's date: ", allows_default=True)
    amount = get_amount()
